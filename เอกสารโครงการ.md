...
- อัปเดตรอบ 2026-03-03 00:00 UTC: ปรับคู่มือ `god_project/neon/คู่มือการใช้งาน-database.md` ให้ใช้ `neon.env` เป็นแหล่งตั้งต้นของ environment (source + map `DATABASE_URL`/`POSTGRES_URL` ไป `NEON_URL`/`SUPABASE_URL`) และแก้เอกสารประกอบให้สอดคล้องเพื่อลดความสับสนเรื่องชื่อไฟล์ env
...
- อัปเดตรอบ 2026-02-28 11:10 UTC: ปรับ `god_project/fetch/calendar/main.py` ให้ loader รองรับไฟล์ `god_project/fetch/neon.env` โดยใช้ `neon.env` ที่ root repo เป็นไฟล์มาตรฐานสำหรับ bootstrap env ก่อนรันคำสั่ง pipeline/migration

- อัปเดตรอบ 2026-02-28 11:58 UTC: ปรับคู่มือ `god_project/neon/คู่มือการใช้งาน-database.md` ในหัวข้อเตรียม environment ให้เป็นขั้นตอนทีละข้อ (แยกกรณี migration SQL กับ pipeline REST) พร้อมตัวอย่าง `export`, การใช้ไฟล์ `neon.env`, และคำสั่งตรวจสอบ env เพื่อช่วยลดความสับสนเวลาเริ่มใช้งาน
