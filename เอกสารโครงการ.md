...
- อัปเดตรอบ 2026-03-03 00:00 UTC: ปรับระบบ Calendar pipeline และเอกสารให้ยกเลิกการใช้ชื่อ Supabase โดยเปลี่ยนไปใช้ `NEON_API_URL`/`NEON_SERVICE_ROLE_KEY`, โหลดไฟล์ `god_project/fetch/neon.env` และอัปเดตคู่มือฐานข้อมูลให้สอดคล้อง end-to-end
...
- เพิ่มไฟล์ตั้งค่า `god project/fetch/calendar/config.yaml` สำหรับกำหนด source endpoint, batch size และการอ่าน credentials จาก env (`NEON_API_URL`, `NEON_SERVICE_ROLE_KEY`)
...
- อัปเดตรอบ 2026-02-28 11:58 UTC: ปรับคู่มือ `god_project/neon/คู่มือการใช้งาน-database.md` ในหัวข้อเตรียม environment ให้เป็นขั้นตอนทีละข้อ (แยกกรณี migration SQL กับ pipeline REST) พร้อมตัวอย่าง `export`, การใช้ไฟล์ `god_project/fetch/neon.env`, และคำสั่งตรวจสอบ env เพื่อช่วยลดความสับสนเวลาเริ่มใช้งาน
