...
- [x] (2026-02-28 10:29 UTC) Hardening: Calendar fetch config bootstrap
  ...
  - เพิ่ม fallback ของ Supabase URL/KEY ให้รองรับทั้งชื่อ env มาตรฐานและ NEXT_PUBLIC
  - หมายเหตุ: production rollout ต้องกำหนด `SUPABASE_SERVICE_ROLE_KEY` ชัดเจนใน secret manager

## 16) บันทึกความคืบหน้าเพิ่มเติม (2026-02-28 10:38 UTC)
- [x] Calendar pipeline UX hardening สำหรับ local run
  - default mode: allow fetch/normalize สำเร็จแม้ไม่มี Supabase credentials (skip DB write พร้อม WARN)
...
## 17) บันทึกแผนปิดประเด็น warning Supabase credential (2026-02-28 10:55 UTC)
...
- แผนงานต่อเนื่อง (สั้น):
  1. จัดทำ `.env.example` สำหรับ pipeline calendar ให้ครบตัวแปรขั้นต่ำ (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`)
...
## 18) อัปเดตแผนย่อย: bootstrap env จาก `supabase.env` (2026-02-28 11:10 UTC)
- [x] เพิ่มแหล่งโหลด env สำหรับ Calendar pipeline: `god_project/fetch/supabase.env`
