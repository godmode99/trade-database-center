...
- [x] แก้ pain point การตั้งค่า Supabase credential ของ Calendar pipeline
...
- ผลลัพธ์: ลด error `Missing Supabase URL/API key in env or config` สำหรับการรันคำสั่งจาก root โปรเจคหรือจากเครื่อง developer ที่เก็บค่าบนไฟล์ env
- งานต่อเนื่อง: แนะนำใช้ `SUPABASE_SERVICE_ROLE_KEY` ใน production เพื่อสิทธิ์ write ที่ถูกต้อง แทน anon key

## 16) บันทึกความคืบหน้าเพิ่มเติม (2026-02-28 10:38 UTC)
- [x] ลด friction ตอนรัน local ของ Calendar pipeline
  - แก้ที่ `god_project/fetch/calendar/main.py` ให้ค่าเริ่มต้น skip ขั้นตอนเขียนฐานข้อมูลเมื่อไม่พบ Supabase credentials (แสดง WARN แทน error)
...
- [x] ทดสอบพฤติกรรม 2 กรณี
  ...
  - ไม่มี credentials + `--require-db` => exit 1 พร้อมข้อความ Missing Supabase URL/API key

## 17) บันทึกวิเคราะห์ปัญหา runtime (2026-02-28 10:55 UTC)
- ตรวจ log ที่ผู้ใช้รายงาน: `Fetched items=85 normalized=85` + `WARN: Missing Supabase URL/API key in env or config, skip database write`
- ข้อสรุป:
  - ขั้นตอน fetch และ normalize ผ่านแล้ว (ไม่มี error ด้านแหล่งข้อมูล)
  - จุดที่ยังไม่ผ่านคือขั้นตอนเขียน Supabase เพราะไม่พบ `SUPABASE_URL` หรือ key สำหรับ write ตาม fallback ที่ระบบรองรับ
  - พฤติกรรมนี้เป็น design ของโหมด default เพื่อให้ local dev รันต่อได้โดยไม่ fail hard
- คำแนะนำใช้งาน:
  - ถ้าต้องการเขียน DB ให้ตั้ง `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (หรือ key fallback ที่รองรับ)
  - ถ้าต้องการให้ job fail ทันทีเมื่อ secret หาย ให้รันพร้อม `--require-db`

## 18) บันทึกการแก้ข้อสงสัยเรื่อง `supabase.env` (2026-02-28 11:10 UTC)
- ปรับโค้ดแล้ว: Calendar pipeline จะพยายามโหลด `god_project/fetch/supabase.env` ในช่วง hydrate env
- ผลเชิงพฤติกรรม: หากมีค่า `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` (หรือ fallback key) อยู่ในไฟล์ดังกล่าว ระบบจะนำไปใช้ต่อในขั้นตอนเขียน DB ได้
