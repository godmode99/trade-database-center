...
- ปรับโค้ดแล้ว: แนวทางปฏิบัติปัจจุบัน: ให้ `source neon.env` จาก root repo แล้ว map env ก่อนรัน pipeline
- ผลเชิงพฤติกรรม: หาก map ค่า `DATABASE_URL`/`POSTGRES_URL` เป็น `NEON_URL`/`SUPABASE_URL` และใส่ `SUPABASE_SERVICE_ROLE_KEY` ครบ ระบบจะเขียน DB ต่อได้
...
- ปิดช่องว่างด้านเอกสาร onboarding environment บางส่วนแล้ว โดยขยายคู่มือฐานข้อมูลที่ `god_project/neon/คู่มือการใช้งาน-database.md` ให้ชัดเจนขึ้นเรื่องตัวแปรที่ต้องใช้จริง (`NEON_URL` สำหรับ psql, `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` สำหรับ pipeline)
- เพิ่ม runbook แบบปฏิบัติได้ทันที: ตัวอย่าง `export`, ทางเลือกใช้ไฟล์ `neon.env`, และคำสั่งตรวจสอบ env ก่อนรัน
...
- ปรับเอกสารคู่มือฐานข้อมูลให้ใช้ `neon.env` เป็นจุดเริ่มต้นของการตั้งค่า environment และกำหนดขั้นตอน source + map ค่าให้ตรงกับตัวแปรที่ pipeline ใช้จริง
- ปรับข้อความอ้างอิงเดิมที่ใช้ `god_project/fetch/supabase.env` ให้สอดคล้องเป็นแนวทางเดียวกันด้วย `neon.env`
