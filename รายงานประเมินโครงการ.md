...
- ข้อสรุป:
  ...
  - จุดที่ยังไม่ผ่านคือขั้นตอนเขียน Neon เพราะไม่พบ `NEON_API_URL` หรือ key สำหรับ write ตาม fallback ที่ระบบรองรับ
  - พฤติกรรมนี้เป็น design ของโหมด default เพื่อให้ local dev รันต่อได้โดยไม่ fail hard
- คำแนะนำใช้งาน:
  - ถ้าต้องการเขียน DB ให้ตั้ง `NEON_API_URL` + `NEON_SERVICE_ROLE_KEY` (หรือ key fallback ที่รองรับ)
...
- ผลเชิงพฤติกรรม: หากมีค่า `NEON_API_URL`/`NEON_SERVICE_ROLE_KEY` อยู่ในไฟล์ดังกล่าว ระบบจะนำไปใช้ต่อในขั้นตอนเขียน DB ได้
...
- อัปเดตรอบ 2026-02-28 11:46 UTC: ปิดงาน Foundation เรื่อง schema+migration v1 โดยมีไฟล์ใช้งานจริงที่ `god_project/neon/migrations/202602280001_schema_v1.sql` ครอบคลุมตาราง ingest ทุกแหล่งข้อมูล, `ops.pipeline_runs`, และ latest views เพื่อรองรับ dashboard/query layer ใน phase ถัดไป

## 19) อัปเดตล่าสุด (2026-02-28 11:51 UTC)
- เพิ่มเอกสารคู่มือการใช้งานฐานข้อมูลที่ `god_project/neon/คู่มือการใช้งาน-database.md` เพื่อปิดช่องว่างด้าน Data Contract/Storage usage ในระดับปฏิบัติการ
- เนื้อหาครอบคลุม: การตั้งค่า `NEON_URL`, การรัน migration, รายการตารางสำคัญ, ตัวอย่าง upsert/query, และแนวทาง monitoring ตาราง `ops.pipeline_runs`
- ผลกระทบเชิง readiness: ช่วยให้ทีม fetch/API/web ใช้มาตรฐานเดียวกันและลดเวลา onboarding เมื่อต้องเชื่อมกับ Neon schema v1

## อัปเดตเพิ่มเติม 2026-02-28 11:58 UTC
- ปิดช่องว่างด้านเอกสาร onboarding environment บางส่วนแล้ว โดยขยายคู่มือฐานข้อมูลที่ `god_project/neon/คู่มือการใช้งาน-database.md` ให้ชัดเจนขึ้นเรื่องตัวแปรที่ต้องใช้จริง (`NEON_URL` สำหรับ psql, `NEON_API_URL`/`NEON_SERVICE_ROLE_KEY` สำหรับ pipeline)
- เพิ่ม runbook แบบปฏิบัติได้ทันที: ตัวอย่าง `export`, ทางเลือกใช้ไฟล์ `god_project/fetch/neon.env`, และคำสั่งตรวจสอบ env ก่อนรัน
- ผลลัพธ์ที่คาดหวัง: ลดความผิดพลาดช่วงเริ่มต้นของทีมพัฒนา/ผู้ใช้งานที่สับสนชื่อ env ระหว่างชั้น SQL และ REST

## อัปเดตเพิ่มเติม 2026-03-03 00:00 UTC
- ปรับโค้ด Calendar pipeline ให้ใช้ชื่อ env ของ Neon โดยตรง (`NEON_API_URL`, `NEON_SERVICE_ROLE_KEY`) และปรับข้อความเตือน/CLI help ให้ไม่อ้าง Supabase
- ปรับคอนฟิก `god_project/fetch/calendar/config.yaml` ให้ค่าเริ่มต้นชี้ env ชุด Neon
- ปรับคู่มือฐานข้อมูล + เอกสารประกอบให้ยกเลิกการอ้าง `SUPABASE_*` และใช้ไฟล์ `god_project/fetch/neon.env` เป็นมาตรฐาน
